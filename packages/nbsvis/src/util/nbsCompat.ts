import { fromArrayBuffer, Instrument, Song, type FromArrayBufferOptions } from '@encode42/nbs.js';
...
export function normalizeNbsSong(song: Song): Song {
  ...
  for (const inst of customs) {
    const targetId = inst.id >= firstCustom ? inst.id : firstCustom + customs.indexOf(inst);
...
export function loadNbsFromBuffer(buffer: ArrayBuffer, options?: FromArrayBufferOptions): Song {
